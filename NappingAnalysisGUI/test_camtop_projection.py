...
# - Détection : HSV sur frame réduite 640×360
...
#         # Charger H_top_to_bottom
#         h_path = pose_path.replace("camtop_table_pose.json",
#                                    "H_top_to_bottom.json")
#         if os.path.isfile(h_path):
#             h_data  = json.load(open(h_path, "r", encoding="utf-8"))
#             self._H = np.array(h_data["H_top_to_bottom"], dtype=np.float32)
#             print("[Pose] H_top_to_bottom chargée ✅")
#         else:
#             self._H = None
#             print("[Pose] ⚠ H_top_to_bottom.json introuvable")

#     def pixel_to_mm(self, u, v):
#         """Pixel natif cam_top → mm repère cam_bottom via H."""
...
#         else:
#             # Fallback inversion XY
#             return (TABLE_SIZE_MM - x_top, TABLE_SIZE_MM - y_top)
...
#     print("q=quitter  d=masque  +/-=seuil  r=reset")
#     show_mask = False
...
#         if show_mask:
#             cv2.imshow("Masque HSV", cv2.resize(mask, (960, 540)))
...
#             print(f"seuil reset → {V_THRESHOLD}")
