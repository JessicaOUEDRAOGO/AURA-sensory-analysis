...
# taille de la mire utile projetée pour aider au clic
DISPLAY_SQUARE_SIZE = 1800
...
def build_display_pattern():
    """
    Mire visuelle uniquement pour mieux voir l'écran.
    Elle ne sert PAS aux calculs géométriques.
    """
    img = np.zeros((PROJECTOR_HEIGHT, PROJECTOR_WIDTH, 3), dtype=np.uint8)

    x0 = (PROJECTOR_WIDTH - DISPLAY_SQUARE_SIZE) // 2
    y0 = (PROJECTOR_HEIGHT - DISPLAY_SQUARE_SIZE) // 2
    x1 = x0 + DISPLAY_SQUARE_SIZE - 1
    y1 = y0 + DISPLAY_SQUARE_SIZE - 1

    cv2.rectangle(img, (x0, y0), (x1, y1), (DISPLAY_BG_LEVEL, DISPLAY_BG_LEVEL, DISPLAY_BG_LEVEL), thickness=-1)
    cv2.rectangle(img, (x0, y0), (x1, y1), (255, 255, 255), thickness=6)

    # petits repères rouges pour aider visuellement
    corners = [(x0, y0), (x1, y0), (x1, y1), (x0, y1)]
    for p in corners:
        cv2.circle(img, p, 14, (0, 0, 255), -1)
...
def open_camera():
    ...
    h, w = frame.shape[:2]
    print(f"Caméra ouverte en {w}x{h}")
...
def refine_click_to_corner(gray, x, y, roi_half_size=25):
    """
    Raffine localement le clic en cherchant un coin fort dans une petite ROI.
    Si rien n'est trouvé, retourne le point cliqué.
    """
    h, w = gray.shape[:2]
...
def mouse_callback(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN and len(clicked_points) < 4:
        rx, ry = refine_click_to_corner(frozen_gray, x, y, roi_half_size=25)
...
def main():
    ...
    try:
        ...
        print("\nClique exactement dans cet ordre :")
        print("1. coin haut gauche du carré visible 59x59")
        print("2. coin haut droit")
        print("3. coin bas droit")
        ...
        cv2.namedWindow("Manual Screen Calibration", cv2.WINDOW_NORMAL)
        cv2.setMouseCallback("Manual Screen Calibration", mouse_callback)

        while True:
            display = draw_points(frozen, clicked_points)
            cv2.imshow("Manual Screen Calibration", display)
            ...
            if key == ord("r"):
                ...
                # points 3D du plan écran (59x59 cm)
                object_points_m = np.array([
                    [0.0, 0.0, 0.0],
                    [SCREEN_SIZE_MM / 1000.0, 0.0, 0.0],
                    [SCREEN_SIZE_MM / 1000.0, SCREEN_SIZE_MM / 1000.0, 0.0],
                    [0.0, SCREEN_SIZE_MM / 1000.0, 0.0],
                ], dtype=np.float32)
                ...
                # pose écran -> caméra
                ok, rvec_sc, tvec_sc = cv2.solvePnP(
                    object_points_m,
                    cam_points_raw,
                    K_cam,
                    dist_cam,
                ...
                # pose écran -> projecteur via stéréo caméra->projecteur
                R_sp = R_cp @ R_sc
                ...
                # calibration_data.json compatible appli
                calib_data = {
                ...
                print("\nCalibration de pose sauvegardée :")
                print(f"- {OUTPUT_CALIB_PATH}")
                print(f"- {OUTPUT_POSE_PATH}")
                print(f"- {OUTPUT_DEBUG_PATH}")
                print("\nPoints caméra bruts =\n", cam_points_raw)
                print("\nPoints caméra undist =\n", cam_points_undist)
                print("\nH_proj =\n", H_proj)
                print("\nH_graph =\n", H_graph)
                print("\nrvec écran->caméra =\n", rvec_sc)
                print("\ntvec écran->caméra =\n", tvec_sc)
