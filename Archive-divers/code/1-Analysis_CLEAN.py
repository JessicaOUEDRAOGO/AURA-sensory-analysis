...
def main():
    ...
    while True:
        ...
        """if qr_point is not None:
            ...
            projector_coords = transform_to_projector(qr_point, H)

            # Convertir les coordonnées projetées en entiers
            cam_x = qr_point[0].astype(int)
            cam_y = qr_point[1].astype(int)
            # Dessiner un cercle rouge sur la frame (au lieu de `image_background`)
            cv2.circle(frame, (cam_x, cam_y), 150, (0, 0, 255), 3)  # Cercle rouge
            # Afficher les coordonnées projetées sur la frame
            cv2.putText(frame, f"{cam_x}, {cam_y}", (cam_x, cam_y),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)

            # Dessiner un cercle rouge autour du QR Code sur l'image de fond
            cv2.circle(display_image, tuple(projector_coords.astype(int)), 150, (0, 0, 255), 5)  # Cercle rouge
        ...
        if ids is not None:
            ...
            min_area = float("inf")
            smallest_aruco = None
            smallest_corners = None  # Stocker les coins du plus petit ArUco

            for i in range(len(ids)):
                aruco_id = ids[i][0]  # ID du marqueur
                corners_array = corners[i][0]  # Coins du marqueur (4 points)

                # Calcul de la largeur et hauteur en pixels
                width = np.linalg.norm(corners_array[0] - corners_array[1])
                height = np.linalg.norm(corners_array[1] - corners_array[2])
                area = width * height  # Surface en pixels

                # Vérification du plus petit ArUco détecté
                if area < min_area:
                    min_area = area
                    smallest_aruco = (aruco_id, width, height)
                    smallest_corners = corners_array  # Sauvegarde des coins

                # Dessiner les marqueurs détectés
                cv2.aruco.drawDetectedMarkers(frame, corners)

            # Tracer un cadre orange autour du plus petit ArUco
            if smallest_corners is not None:
                for j in range(4):  # Boucle sur les 4 coins
                    pt1 = tuple(smallest_corners[j].astype(int))
                    pt2 = tuple(smallest_corners[(j + 1) % 4].astype(int))  # Coin suivant
                    cv2.line(frame, pt1, pt2, (0, 165, 255), 3)  # Orange en BGR

                print(f"Plus petit ArUco détecté: ID={smallest_aruco[0]}, Largeur={smallest_aruco[1]:.2f} px, Hauteur={smallest_aruco[2]:.2f} px")
        ...
        end_p5 = time.time()
        end_loop = time.time()
        print(f"Iter {c}: - nb de tag = {len(ids)}")
        print(f"  - Temps total de la boucle : {end_loop - start_loop:.6f} s - {(end_loop - start_loop)/(end_loop - start_loop)*100:.3f}%")
        #print(f"  - Temps de p1 : {end_p1 - star_p1:.6f} s - {(end_p1 - star_p1)/(end_loop - start_loop)*100:.3f}%")
        print(f"  - Temps de détection : {end_detect - start_detect:.6f} s - {(end_detect - start_detect)/(end_loop - start_loop)*100:.3f}%")
        print(f"  - Temps de p2 : {end_p2 - star_p2:.6f} s - {(end_p2 - star_p2)/(end_loop - start_loop)*100:.3f}%")
        print(f"  - Temps de p3 : {end_p3 - star_p3:.6f} s - {(end_p3 - star_p3)/(end_loop - start_loop)*100:.3f}%")
        print(f"  - Temps de p4 : {end_p4 - star_p4:.6f} s - {(end_p4 - star_p4)/(end_loop - start_loop)*100:.3f}%")
        print(f"  -   - Temps de p4_1 : {end_p4_1 - star_p4_1:.6f} s - {(end_p4_1 - star_p4_1)/(end_p4 - star_p4)*100:.3f}%")
        print(f"  -   - Temps de p4_2 : {end_p4_2 - star_p4_2:.6f} s - {(end_p4_2 - star_p4_2)/(end_p4 - star_p4)*100:.3f}%")
        print(f"  -   - Temps de p4_3 : {end_p4_3 - star_p4_3:.6f} s - {(end_p4_3 - star_p4_3)/(end_p4 - star_p4)*100:.3f}%")
        print(f"  - Temps de p5 : {end_p5 - star_p5:.6f} s - {(end_p5 - star_p5)/(end_loop - start_loop)*100:.3f}%")
        print(f"  -   - Temps de p5_1 : {end_p5_1 - star_p5_1:.6f} s - {(end_p5_1 - star_p5_1)/(end_p5 - star_p5)*100:.3f}%")
        print(f"  -   - Temps de p5_2 : {end_p5_2 - star_p5_2:.6f} s - {(end_p5_2 - star_p5_2)/(end_p5 - star_p5)*100:.3f}%")
        print(f"  -   - Temps de p5_3 : {end_p5_3 - star_p5_3:.6f} s - {(end_p5_3 - star_p5_3)/(end_p5 - star_p5)*100:.3f}%")
        print("\n")
