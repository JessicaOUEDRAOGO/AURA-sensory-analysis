    ...
    if ids is not None:
        with frame_lock:
            ...
            for i in range(len(ids)):
                ...
                all_aruco.append([ids[i][0], np.mean(corners[i][0], axis=0)])  # Stocke l'ID et le centre

def tracking_thread(cap, proj_points):
    """Thread qui gère le suivi des ArUco et déclenche le post-traitement."""
...
def main():
    ...
    # Lancement des threads avec cap en paramètre
    
    setUp_on_main_monitor()
    start_createThread = time.time()
    t1 = threading.Thread(target=tracking_thread, args=(cap, proj_points,))
    end_createThread = time.time()
    ...
    print(f"  - Temps total de la createThread : {end_createThread - start_createThread:.6f} s - {(end_createThread - start_createThread)/(end_createThread - start_createThread)*100:.3f}%")
    print(f"  - Temps total de la start : {end_start - start_start:.6f} s - {(end_start - start_start)/(end_start - start_start)*100:.3f}%")

    start_join = time.time()
    t1.join()
    end_join = time.time()
    t2.join()
    print(f"  - Temps total de la join : {end_join - start_join:.6f} s - {(end_join - start_join)/(end_join - start_join)*100:.3f}%")
